...
class SynapseListObj:
    def __init__( self, skdata, category, proj_opts ):
        ...
        self.connid2nodeid = { dat[1]:dat[0] for dat in skdata[1] if dat[2] == cat_ind }
        self.connid2loc = { dat[1]:dat[3:6] for dat in skdata[1] if dat[2] == cat_ind }
...
class NeuronObj:
    def __init__( self, skid, proj_opts ):
        ...
        self.A = sparse.dok_matrix( (len(self.nodeloc), len(self.nodeloc)), dtype=np.float32 )
        self.Ab = sparse.dok_matrix( (len(self.nodeloc), len(self.nodeloc)), dtype=np.float32 )
        for key in self.nodeparent.keys():
            if self.nodeparent[key] is not None:
                ...
                self.Ab[self.node2ind[key], self.node2ind[ self.nodeparent[key]] ] = 1
...
def dist_to_root( nrn ):
    ...
    D = csgraph.shortest_path( nrn.A.transpose(), directed=True, unweighted=False, method='D' )
...
def find_end_nodes( nrn ):
    ...
    return [nrn.nodeids[ ind ] for ind in np.matrix.tolist( y )[0] ]

def minimal_paths( nrn ):
    # Returns list of lists, the minimally overlapping paths from each end point toward root
    D = dist_to_root( nrn )
    ids_end = find_end_nodes( nrn )

    ends_sorted = [ids_end[ind] for ind in np.argsort(D[[nrn.node2ind[id] for id in ids_end]])[::-1] ]
